...
public class RenderLogItem implements IItemRenderer {
	@Override
	...
	private static void renderItem(RenderBlocks renderer, ItemStack itemStack, float x, float y, float z, boolean fullBlock) {
		...
		renderer.renderFaceYPos(block, 0.0D, 0.0D, 0.0D, heartIcon);
		tessellator.draw();

		// front right
		tessellator.startDrawingQuads();
		tessellator.setNormal(0.0F, 0.0F, 1.0F);
		renderer.renderFaceZPos(block, 0.0D, 0.0D, 0.0D, barkIcon);
		tessellator.draw();

		// front left
		tessellator.startDrawingQuads();
		tessellator.setNormal(-1.0F, 0.0F, 0.0F);
		renderer.renderFaceXNeg(block, 0.0D, 0.0D, 0.0D, barkIcon);
		tessellator.draw();
		...
		if (fullBlock) {
			...
			tessellator.startDrawingQuads();
			tessellator.setNormal(0.0F, -1.0F, 0.0F);
			renderer.renderFaceYNeg(block, 0.0D, 0.0D, 0.0D, heartIcon);
			tessellator.draw();

			// back right
			tessellator.startDrawingQuads();
			tessellator.setNormal(0.0F, 0.0F, -1.0F);
			renderer.renderFaceZNeg(block, 0.0D, 0.0D, 0.0D, barkIcon);
			tessellator.draw();

			// back left
			tessellator.startDrawingQuads();
			tessellator.setNormal(1.0F, 0.0F, 0.0F);
			renderer.renderFaceXPos(block, 0.0D, 0.0D, 0.0D, barkIcon);
			tessellator.draw();
		}
